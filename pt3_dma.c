...
pt3_dma_build_page_descriptor(PT3_DMA *dma, int loop)
{
	...
	if (unlikely(dma == NULL)) {
		PT3_PRINTK(&dma->pdev->dev, 1, KERN_ERR, "dma build page descriptor needs DMA\n");
		return;
	}
#if 0
	PT3_PRINTK(&dma->pdev->dev, 7, KERN_DEBUG, "build page descriptor ts_count=%d ts_size=0x%x desc_count=%d desc_size=0x%x\n",
	...
	if (unlikely(desc_info == NULL)) {
		PT3_PRINTK(&dma->pdev->dev, 0, KERN_ERR, "dma maybe failed allocate desc_info %d\n",
				desc_info_pos);
		return;
	}
#endif
	...
	if (unlikely(curr == NULL)) {
		PT3_PRINTK(&dma->pdev->dev, 0, KERN_ERR, "dma maybe failed allocate desc_info->data %d\n",
				desc_info_pos);
		return;
	}
#endif
	...
	for (i = 0; i < dma->ts_count; i++) {
		...
		if (unlikely(dma->ts_count <= ts_info_pos)) {
			PT3_PRINTK(&dma->pdev->dev, 0, KERN_ERR, "ts_info overflow max=%d curr=%d\n",
		...
		if (unlikely(ts_info == NULL)) {
			PT3_PRINTK(&dma->pdev->dev, 0, KERN_ERR, "dma maybe failed allocate ts_info %d\n",
					ts_info_pos);
			return;
		}
#endif
		...
		for (j = 0; j < ts_size / DMA_PAGE_SIZE; j++) {
			if (desc_remain < DMA_DESC_SIZE) {
				...
				if (unlikely(dma->desc_count <= desc_info_pos)) {
					PT3_PRINTK(&dma->pdev->dev, 0, KERN_ERR, "desc_info overflow max=%d curr=%d\n",
				...
				/*
				PT3_PRINTK(&dma->pdev->dev, 7, KERN_DEBUG, "desc_info_pos=%d ts_addr=0x%llx remain=%d\n",
...
#if 0
			PT3_PRINTK(&dma->pdev->dev, 7, KERN_DEBUG, "dma write desc ts_addr=0x%llx desc_info_pos=%d\n",
			...
			if (unlikely(desc_info->size <= desc_info->data_pos)) {
				PT3_PRINTK(&dma->pdev->dev, 0, KERN_ERR, "dma desc_info data overflow.\n");
				return;
			}
			curr = &desc_info->data[desc_info->data_pos];
...
pt3_dma_set_test_mode(PT3_DMA *dma, int test, __u16 init, int not, int reset)
{
	...
	PT3_PRINTK(&dma->pdev->dev, 7, KERN_DEBUG, "set_test_mode base=%p data=0x%04d\n",
			base, data);
...
pt3_dma_set_enabled(PT3_DMA *dma, int enabled)
{
	...
	if (enabled) {
		PT3_PRINTK(&dma->pdev->dev, 7, KERN_DEBUG, "enable dma real_index=%d start_addr=%llx\n",
		...
		PT3_PRINTK(&dma->pdev->dev, 7, KERN_DEBUG, "set descriptor address low %llx\n",
				BIT_SHIFT_MASK(start_addr,  0, 32));
		PT3_PRINTK(&dma->pdev->dev, 7, KERN_DEBUG, "set descriptor address heigh %llx\n",
...
pt3_dma_copy(PT3_DMA *dma, char __user *buf, size_t size, loff_t *ppos, int look_ready)
{
	...
	PT3_PRINTK(&dma->pdev->dev, 7, KERN_DEBUG, "dma_copy ts_pos=0x%x data_pos=0x%x\n",
	...
	for (;;) {
		if (likely(look_ready)) {
			...
			if (dma->ts_info[prev].data[0] != NOT_SYNC_BYTE)
				PT3_PRINTK(&dma->pdev->dev, 7, KERN_INFO, "dma buffer overflow. prev=%d data=0x%x\n",
		...
		for (;;) {
			...
			if (pt3_copy_to_user(&buf[size - remain], &page->data[page->data_pos], csize)) {
...
pt3_dma_ready(PT3_DMA *dma)
{
	...
	PT3_PRINTK(&dma->pdev->dev, 0, KERN_DEBUG, "invalid sync byte value=0x%02x ts_pos=%d data_pos=%d curr=0x%02x\n",
...
create_pt3_dma(struct pci_dev *hwdev, PT3_I2C *i2c, int real_index)
{
	...
	if (dma == NULL) {
		dev_err(&hwdev->dev, "PT3: fail allocate PT3_DMA\n");
		goto fail;
	}
	...
	if (dma->ts_info == NULL) {
		PT3_PRINTK(&hwdev->dev, 0, KERN_ERR, "fail allocate PT3_DMA_PAGE\n");
		goto fail;
	}
	for (i = 0; i < dma->ts_count; i++) {
		page = &dma->ts_info[i];
	...
	PT3_PRINTK(&hwdev->dev, 7, KERN_DEBUG, "Allocate TS buffer.\n");
	...
	PT3_PRINTK(&dma->pdev->dev, 7, KERN_DEBUG, "Allocate Descriptor buffer.\n");
	pt3_dma_build_page_descriptor(dma, 1);
	PT3_PRINTK(&dma->pdev->dev, 7, KERN_DEBUG, "set page descriptor.\n");
#if 0
