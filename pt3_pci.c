...
check_fpga_version(PT3_DEVICE *dev_conf)
{
	...
	if (dev_conf->version.ptn != 3) {
		PT3_PRINTK(NULL, 0, KERN_ERR, "Not a PT3\n");
		return -1;
	}
	PT3_PRINTK(NULL, 7, KERN_INFO, "PT%d found\n", dev_conf->version.ptn);

	if (dev_conf->version.fpga != 0x04) {
		PT3_PRINTK(NULL, 0, KERN_ERR, "FPGA version 0x%x is not supported\n",
...
set_frequency(int isdb, PT3_TUNER *tuner, __u32 channel, __s32 offset)
{
	...
	PT3_PRINTK(NULL, 7, KERN_DEBUG, "set_freq isdb=%d tuner_no=%d channel=%d offset=%d\n",
...
set_tuner_sleep(int isdb, PT3_TUNER *tuner, int sleep)
{
	...
	switch (isdb) {
	case PT3_ISDB_S :
		PT3_PRINTK(NULL, 1, KERN_INFO, "TUNER %p ISDB_S %s\n", tuner,
					(sleep) ? "Sleep" : "Wakeup");
		status = pt3_qm_set_sleep(tuner->qm, sleep);
		break;
	case PT3_ISDB_T :
		PT3_PRINTK(NULL, 1, KERN_INFO, "TUNER %p ISDB_T %s\n", tuner,
					(sleep) ? "Sleep" : "Wakeup");
...
init_tuner(PT3_I2C *i2c, PT3_TUNER *tuner)
{
	...
	pt3_qm_init_reg_param(tuner->qm);
	{
		...
		free_pt3_bus(bus);
		if (status) {
			PT3_PRINTK(NULL, 7, KERN_DEBUG, "fail init_tuner dummy reset. status=0x%x\n", status);
			return status;
		}
	}

	{
		...
		free_pt3_bus(bus);
		if (status) {
			PT3_PRINTK(NULL, 7, KERN_DEBUG, "fail init_tuner qm init. status=0x%x\n", status);
			return status;
		}
	}
...
tuner_power_on(PT3_DEVICE *dev_conf, PT3_BUS *bus)
{
	...
	for (i = 0; i < MAX_TUNER; i++) {
		...
		if (status)
			PT3_PRINTK(NULL, 1, KERN_INFO, "tc_init_s[%d] status=0x%x\n", i, status);
	}
	for (i = 0; i < MAX_TUNER; i++) {
		tuner = &dev_conf->tuner[i];
		status = pt3_tc_init_t(tuner->tc_t, NULL);
		if (status)
			PT3_PRINTK(NULL, 1, KERN_INFO, "tc_init_t[%d] status=0x%x\n", i, status);
	}

	tuner = &dev_conf->tuner[1];
	status = pt3_tc_set_powers(tuner->tc_t, NULL, 1, 0);
	if (status) {
		PT3_PRINTK(NULL, 7, KERN_DEBUG, "fail set powers.\n");
		goto last;
	}
	...
	for (i = 0; i < MAX_TUNER; i++) {
		...
		if (status)
			PT3_PRINTK(NULL, 1, KERN_INFO, "fail set ts pins mode s [%d] status=0x%x\n", i, status);
	}
	for (i = 0; i < MAX_TUNER; i++) {
		tuner = &dev_conf->tuner[i];
		status = pt3_tc_set_ts_pins_mode_t(tuner->tc_t, NULL, &pins);
		if (status)
			PT3_PRINTK(NULL, 1, KERN_INFO, "fail set ts pins mode t [%d] status=0x%x\n", i, status);
	}
	...
	for (i = 0; i < MAX_TUNER; i++) {
		for (j = 0; j < 10; j++) {
			if (j != 0)
				PT3_PRINTK(NULL, 0, KERN_INFO, "retry init_tuner\n");
		...
		if (status) {
			PT3_PRINTK(NULL, 7, KERN_INFO, "fail init_tuner %d status=0x%x\n", i, status);
			goto last;
		}
	}
	...
	if (status) {
		PT3_PRINTK(NULL, 7, KERN_INFO, "failed inst_addr=0x%x status=0x%x\n",
	...
	if (status) {
		 PT3_PRINTK(NULL, 7, KERN_INFO, "fail tc_set_powers,\n");
		goto last;
	}
...
init_all_tuner(PT3_DEVICE *dev_conf)
{
	...
	if (!pt3_i2c_is_clean(i2c)) {
		PT3_PRINTK(NULL, 0, KERN_INFO, "cleanup I2C bus.\n");
	...
	if (status)
		goto last;
	PT3_PRINTK(NULL, 7, KERN_DEBUG, "tuner_power_on\n");
	...
	for (i = 0; i < MAX_TUNER; i++) {
		for (j = 0; j < PT3_ISDB_MAX; j++) {
			...
			if (status) {
				PT3_PRINTK(NULL, 0, KERN_DEBUG, "fail set_frequency. status=0x%x\n", status);
			}
...
get_cn_agc(PT3_CHANNEL *channel, __u32 *cn, __u32 *curr_agc, __u32 *max_agc)
{
	...
	PT3_PRINTK(NULL, 7, KERN_INFO, "cn=0x%x\n", *cn);
	PT3_PRINTK(NULL, 7, KERN_INFO, "agc=0x%x\n", *curr_agc);
...
SetChannel(PT3_CHANNEL *channel, FREQUENCY *freq)
{
	...
	case PT3_ISDB_S :
		...
		if (status) {
			PT3_PRINTK(NULL, 1, KERN_ERR, "fail get_tmcc_s status=0x%x\n", status);
			return status;
		}
		PT3_PRINTK(NULL, 7, KERN_DEBUG, "tmcc_s.id = 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",
		...
		if (status) {
			PT3_PRINTK(NULL, 1, KERN_ERR, "fail set_tmcc_s status=0x%x\n", status);
			return status;
		}
		for (i = 0; i < 1000; i++) {
			status = get_id_s(channel->tuner, &tsid);
			if (status) {
				PT3_PRINTK(NULL, 1, KERN_ERR, "fail get_id_s status=0x%x\n", status);
				return status;
			}
			PT3_PRINTK(NULL, 7, KERN_DEBUG, "tsid=0x%x\n", tsid);
	...
	case PT3_ISDB_T :
		...
		if (status) {
			PT3_PRINTK(NULL, 1, KERN_ERR, "fail get_tmcc_t status=0x%x\n", status);
			return status;
		}
		// wait for fill buffer
...
pt3_open(struct inode *inode, struct file *file)
{
	...
	for (lp = 0; lp < MAX_PCI_DEVICE; lp++) {
		if (device[lp] == NULL) {
			PT3_PRINTK(NULL, 1, KERN_DEBUG, "device does not exist\n");
			return -EIO;
		}
			...
			for (lp2 = 0; lp2 < MAX_CHANNEL; lp2++) {
				...
				if (channel->minor == minor) {
					if (channel->valid) {
						...
						PT3_PRINTK(NULL, 1, KERN_DEBUG, "device is already used.\n");
						return -EIO;
					}
					PT3_PRINTK(NULL, 7, KERN_DEBUG, "selected tuner_no=%d type=%d\n",
...
pt3_release(struct inode *inode, struct file *file)
{
	...
	if (debug > 0)
		PT3_PRINTK(NULL, 0, KERN_INFO, "(%d:%d) error count %d\n",
...
pt3_read(struct file *file, char __user *buf, size_t cnt, loff_t * ppos)
{
	...
	if (rcnt < 0) {
		PT3_PRINTK(&channel->ptr->pdev->dev, 1, KERN_INFO, "fail copy_to_user.\n");
		return -EFAULT;
	}
...
count_used_bs_tuners(PT3_DEVICE *device)
{
	...
	PT3_PRINTK(NULL, 1, KERN_INFO, "used bs tuners on %p = %d\n", device, count);
...
pt3_do_ioctl(struct file  *file, unsigned int cmd, unsigned long arg0)
{
	...
	switch (cmd) {
	case SET_CHANNEL:
		if (pt3_copy_from_user(&freq, arg, sizeof(FREQUENCY)))
			return -EFAULT;
	...
	case GET_SIGNAL_STRENGTH:
		...
		if (status)
			PT3_PRINTK(&channel->ptr->pdev->dev, 1, KERN_INFO, "fail get signal strength status=0x%x\n", status);
		if (pt3_copy_to_user(arg, &signal, sizeof(int)))
			return -EFAULT;
		return 0;
	case LNB_ENABLE:
		...
		if (count <= 1 && !lnb_force) {
			...
			PT3_PRINTK(NULL, 1, KERN_INFO, "LNB on %s\n", voltage[lnb_eff]);
		}
		return 0;
	case LNB_DISABLE:
		count = count_used_bs_tuners(channel->ptr);
	...
	case GET_TS_ERROR_PACKET_COUNT:
		...
		if (pt3_copy_to_user(arg, &count, sizeof(unsigned int)))
			return -EFAULT;
		return 0;
	}
	return -EINVAL;
...
pt3_pci_init_one (struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	pci_set_master(pdev);
	PT3_PRINTK(NULL, 0, KERN_INFO, "Bus Mastering Enabled.\n");
