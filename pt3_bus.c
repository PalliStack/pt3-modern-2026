...
add_instruction(PT3_BUS *bus, __u32 instruction)
{
	...
	if (bus->inst_count % 2) {
		...
		if (bus->inst_pos >= sizeof(bus->insts)) {
			PT3_PRINTK(NULL, 0, KERN_ERR, "bus instructions is over flow\n");
			bus->inst_pos = 0;
		}
	}
	bus->inst_count++;
...
datan(PT3_BUS *bus, __u32 index, __u32 n)
{
	...
	if (unlikely(bus->buf == NULL)) {
		PT3_PRINTK(NULL, 0, KERN_ERR, "buf is not ready.\n");
		return 0;
	}
	if (unlikely(bus->buf_size < index + n)) {
		PT3_PRINTK(NULL, 0, KERN_ERR, "buf does not  have enough size. buf_size=%u\n",
				bus->buf_size);
		return 0;
	}
...
pt3_bus_read(PT3_BUS *bus, __u8 *data, __u32 size)
{
	...
	if (likely(bus->buf == NULL)) {
		...
		PT3_PRINTK(NULL, 0, KERN_ERR, "bus read buff is already exists.\n");
...
pt3_bus_push_read_data(PT3_BUS *bus, __u8 data)
{
	if (unlikely(bus->buf != NULL)) {
		if (bus->buf_pos >= bus->buf_size) {
			PT3_PRINTK(NULL, 0, KERN_ERR, "buffer over run. pos=%u\n", bus->buf_pos);
			bus->buf_pos = 0;
		}
...
#if 0
	PT3_PRINTK(NULL, 7, KERN_DEBUG, "bus read data=0x%02x\n", data);
#endif
}
