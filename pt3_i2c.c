...
run_code(PT3_I2C *i2c, __u32 start_addr, __u32 *ack)
{
	...
	if (ack != NULL)
		*ack = a;
	if (a)
		PT3_PRINTK(i2c->dev, 0, KERN_DEBUG, "fail i2c run_code status 0x%x\n", data);
...
#if 0
	PT3_PRINTK(i2c->dev, 7, KERN_DEBUG, "PT3 : i2c_copy. base=%p dst=%p src=%p size=%d\n",
...
pt3_i2c_run(PT3_I2C *i2c, PT3_BUS *bus, __u32 *ack, int copy)
{
	...
	if (rsize > 0) {
		for (i = 1; i < 10; i++) {
			PT3_PRINTK(i2c->dev, 7, KERN_DEBUG, "bus_read_data + %d = 0x%x inst = 0x%x\n",
...
PT3_I2C *
create_pt3_i2c(struct device *dev, __u8 __iomem *bar[])
{
	PT3_I2C *i2c;

	i2c = pt3_vzalloc(sizeof(PT3_I2C));
	if (i2c == NULL) {
		dev_err(dev, "PT3: fail allocate PT3_I2C\n");
		goto fail;
	}
